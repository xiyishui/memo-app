...
export async function POST(request) {
  try { const u = await getUser(request); if (!u) return NextResponse.json({e:'x'},{status:401}); const { text } = await request.json(); if (!text||!text.trim()) return NextResponse.json({e:'x'},{status:400}); const { data, error: insErr } = await supabase.from('todos').insert({id:Date.now(),userid:u.id,text:text.trim(),done:false,createdat:new Date().toISOString()}).select().single(); if (insErr || !data) return NextResponse.json({e:insErr?.message||'insert failed'},{status:400}); return NextResponse.json(data,{status:201}); } catch { return NextResponse.json({e:'x'},{status:500}); }
}
